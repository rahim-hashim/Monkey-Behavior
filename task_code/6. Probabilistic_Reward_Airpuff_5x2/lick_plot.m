function [A_lick_rolling_avg, B_lick_rolling_avg, C_lick_rolling_avg, D_lick_rolling_avg, E_lick_rolling_avg] = lick_plot(TrialRecord)
    ...
    if min(len_elements) >= trial_avg
        ...
        % fractal E
        E_lick_rolling_avg = movmean(lick_rate_struct.E, [trial_avg 0]);
        E_blink_rolling_avg = movmean(blink_rate_struct.E, [trial_avg 0]);
    end
    hold on
    if min(len_elements) >= trial_avg
        ...
        E_x = linspace(1, numel(E_lick_rolling_avg), numel(E_lick_rolling_avg));

        % Plot 1
        % plot lick data
        subplot(2,1,1);
        hold on
        plot(A_x, A_lick_rolling_avg, 'r', 'DisplayName','A','LineWidth',3)
        plot(B_x, B_lick_rolling_avg, 'b', 'DisplayName','B','LineWidth',3)
        plot(C_x, C_lick_rolling_avg, 'g', 'DisplayName','C','LineWidth',3)
        plot(D_x, D_lick_rolling_avg, 'm', 'DisplayName','D','LineWidth',3)
        plot(E_x, E_lick_rolling_avg, 'c', 'DisplayName','E','LineWidth',3)
        %customize plot
        yticks('auto')
        xticks('auto')
        xlabel('Trial #')
        ylabel('Avg Lick')
        ylim([0 5]) 
        legend({'A', 'B', 'C', 'D', 'E'}, 'Location', 'westoutside')

        % Plot 2
        % plot blink data
        subplot(2,1,2);
        hold on
        plot(A_x, A_blink_rolling_avg, 'r', 'DisplayName','A','LineWidth',3)      
        plot(B_x, B_blink_rolling_avg, 'b', 'DisplayName','B','LineWidth',3)
        plot(C_x, C_blink_rolling_avg, 'g', 'DisplayName','C','LineWidth',3)
        plot(D_x, D_blink_rolling_avg, 'm', 'DisplayName','D','LineWidth',3)
        plot(E_x, E_blink_rolling_avg, 'c', 'DisplayName','E','LineWidth',3)
        % customize plot
        yticks('auto')
        xticks('auto')
        xlabel('Trial #')
